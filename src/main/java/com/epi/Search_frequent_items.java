...
class Search_frequent_items{
  ...
  // @include
  public static ArrayList<String> search_frequent_items(ArrayList<String> in, int k) {
    ...
    while (sin.hasNext()) {
      buf = sin.next();
      hash.put(buf, hash.containsKey(buf)?hash.get(buf)+1:1); ++n;     
  ...
  public static void check_ans(ArrayList<String> stream, int k, ArrayList<String> items) {
  ...
  public static void main(String[] argv) {
    ...
    for (int times = 0; times < 1000; ++times) {
      ...
      for (int i = 0; i < n; ++i) {        
        stream.add(rand_string(gen.nextInt(5) + 1));
      }
      ArrayList<String> items = search_frequent_items(stream, k);
