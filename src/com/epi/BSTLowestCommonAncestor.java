...
public class BSTLowestCommonAncestor {
    // @include
    public static <T extends Comparable<T>> BinaryTree<T> find_LCA(BinaryTree<T> x,
                                  BinaryTree<T> s,
                                  BinaryTree<T> b) {
        BinaryTree<T> p = x;
        while (p.getData().compareTo(s.getData()) < 0 || p.getData().compareTo(b.getData()) > 0) {
    ...
    public static void main(String[] args) {
        ...
        assert(3 == find_LCA(root, root.getLeft().getLeft(), root.getRight().getLeft()).getData());
        assert(5 == find_LCA(root, root.getRight().getLeft(), root.getRight().getRight()).getData());
        assert(2 == find_LCA(root, root.getLeft().getLeft(), root.getLeft()).getData());
    }
