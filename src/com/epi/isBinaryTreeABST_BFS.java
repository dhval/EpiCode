 ...
 */
public class isBinaryTreeABST_BFS {
    // @include
    ...
    public static <T extends Comparable<T>> boolean is_BST(BinaryTree<T> n, T min, T max) {
        ...
        while (!q.isEmpty()) {
            if (q.getFirst().node != null) {
                ...
                q.addLast(new QNode<T>(q.getFirst().node.getRight(), q.getFirst().node.getData(),
                        q.getFirst().upper));
            }
            q.removeFirst();
        }
