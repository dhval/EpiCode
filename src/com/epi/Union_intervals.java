...
class ExInterval implements Comparable<ExInterval> {
 ...
 public int compareTo(ExInterval i){
   if (left.val < i.left.val) return -1;
   if (left.val > i.left.val) return 1;
   if (left.isClose && !i.left.isClose) return -1;
   if (!left.isClose && i.left.isClose) return 1;
   return 0;
  }
...
class Union_intervals {
  
  public static ArrayList<ExInterval> Union_ExIntervals(ArrayList<ExInterval> I) {
  ...
  public static void check_ExIntervals(ArrayList<ExInterval> A) {
    // only check the ExIntervals do not overlap with each other.
    for (int i = 1; i < A.size(); ++i) {
      assert(A.get(i - 1).right.val < A.get(i).left.val ||
             (A.get(i - 1).right.val == A.get(i).left.val && !A.get(i - 1).right.isClose &&
              !A.get(i).left.isClose));
    }
  }
  ...
  public static void main(String[] argv) {
    ...
    for (int times = 0; times < 1000; ++times) {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        temp.left.isClose = gen.nextBoolean(); temp.left.val = gen.nextInt(9999);        
        temp.right.isClose = gen.nextBoolean(); temp.right.val = gen.nextInt(temp.left.val + 100) + temp.left.val + 1;
        A.add(temp);
      }
      ArrayList<ExInterval> ret = Union_ExIntervals(A);      
      if (!ret.isEmpty()) {
        check_ExIntervals(ret);
      }
    }    
  }
}
