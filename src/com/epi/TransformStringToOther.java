...
public class TransformStringToOther {
    ...
    public static int transform_string(Set<String> D,
                         String s,
                         String t) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < str.length(); ++i) {
                ...
                for (int j = 0; j < 26; ++j) {  // iterates through 'a' ~ 'z'.
                    String modStr = strStart + (char)('a' + j) + strEnd; // change the (i + 1)-th char of str.
    ...
    public static void main(String[] args) {
        ...
        String s = rand_string(len);
        String t = rand_string(len);
        HashSet<String> D = new HashSet<String>();
        D.add(s);
        D.add(t);
        int n = r.nextInt(1000000) + 1;
        for (int i = 0; i < n; ++i) {
            D.add(rand_string(len));
        }
        System.out.println(D);
        System.out.println(s + " " + t + " " + D.size());
        System.out.println(transform_string(D, s, t));
    }
}
