...
public class ShortestPathFewestEdges {
    ...
    public static class GraphVertex implements Comparable<GraphVertex>{
        ...
        public Pair<Integer, Integer> distance = new Pair<Integer, Integer>(Integer.MAX_VALUE, 0);
    ...
    public static void Dijkstra_shortest_path(GraphVertex s, GraphVertex t) {
        // Initialization the distance of starting point.
        s.distance = new Pair<Integer, Integer>(0, 0);
        TreeSet<GraphVertex> node_set = new TreeSet<GraphVertex>();
        node_set.add(s);

        while (!node_set.isEmpty()) {
            // Extract the minimum distance vertex from heap.
            GraphVertex u = node_set.first();
            if (u.equals(t)) {
                break;
            }
            node_set.remove(node_set.first());

            // Relax neighboring vertices of u.
            for (Pair<GraphVertex, Integer> v : u.edges) {
                int v_distance = u.distance.getFirst() + v.getSecond();
                int v_num_edges = u.distance.getSecond() + 1;
                if (v.getFirst().distance.getFirst() > v_distance ||
                        (v.getFirst().distance.getFirst() == v_distance &&
                                v.getFirst().distance.getSecond() > v_num_edges)) {
                    node_set.remove(v.getFirst());
                    v.getFirst().pred = u;
                    v.getFirst().distance = new Pair<Integer, Integer>(v_distance, v_num_edges);
                    node_set.add(v.getFirst());
                }
            }
        }
    ...
    private static void test() {
        ...
        Dijkstra_shortest_path(G.get(s), G.get(t));
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < G.size(); ++i) {
            ...
            for (Pair<GraphVertex, Integer> e : G.get(i).edges) {
                System.out.print(e.getFirst().id + "-" + G.get(0).id + " " + e.getSecond() + ",");
            }
            System.out.println();
        }
