...
public class SearchMaze {
    ...
    public static LinkedList<Coordinate> search_maze(int maze[][],
                                        Coordinate s,
                                        Coordinate e) {
    ...
    private static boolean search_maze_helper(int maze[][],
                                              Coordinate cur,
                                              Coordinate e,
        ...
        List<? extends List<Integer>> shift = Arrays.asList(
    ...
    public static void main(String[] args) {
        ...
        for (int times = 0; times < 1000; ++times) {
            ...
            if (white.size() != 0) {
                ...
                LinkedList<Coordinate> path = search_maze(maze, white.get(start), white.get(end));
                if (!path.isEmpty()) {
                    assert(white.get(start).equals(path.peekFirst()) && white.get(end).equals(path.peekLast()));
                }
                Coordinate prev = null;
                for (Coordinate curr : path) {
                    if (prev != null) {
                        assert(Math.abs(prev.x - curr.x) +
