...
import static com.drx.epi.QueueWithMaxUsingDeque.Queue;
...
public class SlidingWindow {
    ...
    public static void TrafficVolumes(List<TrafficElement> A, int w) {
        Queue<TrafficElement> Q = new Queue<TrafficElement>();
        for(int i = 0; i < A.size(); i++) {
            Q.enqueue(A.get(i));
            while(A.get(i).getTime() - Q.head().getTime() > w) {
                Q.dequeue();
            }
            System.out.println("Max after inserting " + i + " is " + Q.max().getVolume());
        }
    }
    // @exclude
    ...
    public static void main(String[] args) {
        ...
        List<TrafficElement> A = Arrays.asList(new TrafficElement(0, 0), new TrafficElement(1, 1),
                new TrafficElement(2, 3), new TrafficElement(3, 1),
                new TrafficElement(4, 0), new TrafficElement(5, 2),
