...
public class LowestCommonAncestorTemplate {
    // @include
    private static <T> int get_depth(BinaryTree<T> n) {
        int d = 0;
        while(n != null) {
            ++d;
            n = n.getParent();
    ...
    public static <T> BinaryTree<T> LCA(BinaryTree<T> i, BinaryTree<T> j) {
        ...
        int depth_diff = Math.abs(depth_i - depth_j);
        while(depth_diff-- > 0) {
            i = i.getParent();
        }
    ...
    public static void main(String[] args) {
        ...
        root.getLeft().setLeft(new BinaryTree<Integer>(1, null, null, root.getLeft()));
        root.setRight(new BinaryTree<Integer>(5, null, null, root));
        root.getRight().setLeft(new BinaryTree<Integer>(4, null, null, root.getRight()));
        root.getRight().setRight(new BinaryTree<Integer>(6, null, null, root.getRight()));

        // should output 3
        assert(LCA(root.getLeft(), root.getRight()).getData().equals(3));
        System.out.println(LCA(root.getLeft(), root.getRight()).getData());
        // should output 5
        assert(LCA(root.getRight().getLeft(), root.getRight().getRight()).getData().equals(5));
        System.out.println(LCA(root.getRight().getLeft(), root.getRight().getRight()).getData());
        // should output 3
